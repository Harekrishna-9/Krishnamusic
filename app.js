...
const KM_DEFAULT_CFG={featuredSongId:"",playlists:[{id:"highway",name:"Highway Vibes",songIds:[],published:true}],vibes:[],hornEnabled:true,hornVolume:22,defaultVolume:80,defaultShuffle:false,siteTitle:"Krishna Music",siteTagline:"सफ़र लंबा हो, संगीत साथ हो।",highwayStatus:"ON THE HIGHWAY",playlistTitle:"Playlist",autoplayPref:false,repeatPref:true,badgeText:"NOW PLAYING"};
...
function getPublishedMusicFolders(){
  const list=Array.isArray(kmCfg.playlists)?kmCfg.playlists:[];
  return list.filter(p=>p&&p.published===true&&p.id&&p.name);
}

function setPlaylistHeader(mode="songs"){
  const head=document.querySelector(".playlist-file-header");
  if(!head)return;
  if(mode==="folders"){
    head.innerHTML="<span>FOLDER NAME</span><span>TRACKS</span><span></span>";
  }else{
    head.innerHTML="<span>NAME</span><span>ARTIST</span><span></span>";
  }
}

function renderMusicFolders(){
  const folders=getPublishedMusicFolders();
  setPlaylistHeader("folders");
  playlistList.innerHTML="";
  activeFolderId="";

  if(!folders.length){
    playlistList.innerHTML=`<div class="playlist-empty-folder"><span>📁</span><strong>No published music folders</strong><small>Admin → Playlists से playlist publish करें</small></div>`;
  }else{
    folders.forEach(folder=>{
      const validIds=(folder.songIds||[]).filter(id=>playlist.some(track=>track.id===id));
      const btn=document.createElement("button");
      btn.type="button";
      btn.className="music-folder-item";
      btn.innerHTML=`
        <span class="music-folder-icon">📁</span>
        <span class="music-folder-copy">
          <strong>${escapePlaylistText(folder.name)}</strong>
          <small>Krishna Music Playlist</small>
        </span>
        <span class="music-folder-count">${validIds.length}</span>
        <span class="music-folder-arrow">›</span>`;
      btn.onclick=()=>{
        activeFolderId=folder.id;
        playlistFilter="folder:"+folder.id;
        renderPlaylist();
      };
  ...
  if(countEl)countEl.textContent=`${folders.length} folder${folders.length===1?"":"s"}`;
}

function escapePlaylistText(value=""){
  return String(value).replace(/[&<>"']/g,ch=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[ch]));
}

function renderFolderSongs(folderId){
  const folder=getPublishedMusicFolders().find(p=>p.id===folderId);
  setPlaylistHeader("songs");
  playlistList.innerHTML="";

  if(!folder){
    playlistFilter="folders";
    renderMusicFolders();
    return;
  }

  const ids=new Set(folder.songIds||[]);
  const entries=playlist.map((track,index)=>({track,index})).filter(({track})=>ids.has(track.id));

  const back=document.createElement("button");
  back.type="button";
  back.className="music-folder-back";
  back.innerHTML=`<span>←</span><div><strong>${escapePlaylistText(folder.name)}</strong><small>Back to Music Folders</small></div>`;
  back.onclick=()=>{playlistFilter="folders";activeFolderId="";renderPlaylist()};
  playlistList.appendChild(back);

  if(!entries.length){
    const empty=document.createElement("div");
    empty.className="playlist-empty-folder";
    empty.innerHTML="<span>♫</span><strong>This folder is empty</strong><small>Admin Playlist Builder से songs add करें</small>";
  ...
  const countEl=document.getElementById("playlistCount");
  if(countEl)countEl.textContent=`${entries.length} song${entries.length===1?"":"s"} • ${folder.name}`;
}

function renderPlaylist(){
  if(playlistFilter==="folders"){
    renderMusicFolders();
    return;
  }
  if(playlistFilter.startsWith("folder:")){
    renderFolderSongs(playlistFilter.slice(7));
    return;
  }
...
playlistTabs.forEach(tab=>tab.addEventListener("click",()=>{
  ...
  if(playlistFilter!=="folders")activeFolderId="";
  renderPlaylist();
}));
