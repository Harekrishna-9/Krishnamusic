...
function close(){document.body.style.overflow="";$("modal").classList.add("hidden");$("songForm").reset();$("active").checked=true;$("editId").value="";$("audioName").textContent="MP3, WAV, M4A";$("coverName").textContent="JPG, PNG, WEBP";$("saveBtn").classList.remove("loading");$("saveBtn").disabled=false;document.querySelector(".save-text").textContent="Save Music";$("mediaPreview").classList.add("hidden");$("adminPreviewAudio").pause();if(previewObjectUrl){URL.revokeObjectURL(previewObjectUrl);previewObjectUrl=""}}
$("add").onclick=()=>{close();$("modalTitle").textContent="Add Music";open()};$("close").onclick=close;$("cancel").onclick=close;$("modal").onclick=e=>{if(e.target===$("modal"))close()};document.addEventListener("keydown",e=>{if(e.key==="Escape"&&!$("modal").classList.contains("hidden"))close()});
